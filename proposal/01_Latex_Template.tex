...
In Figure \ref{ann}, images in the first row are part of the training dataset and below them are the annotations. These images are not directly used for the training of algorithm. Like mentioned before the result of annotations (json) is projected to get the masks of craters. These masks are binary images that represents the black and white pixel values of images. White pixels means craters and black means background class as shown in the Figure \ref{mask}. These binary masks have pixel values ranging from 0 to 255.\\\\
